...
class BackgroundLocationService {
  ...
  // تم التعديل: جعلناه final لتجنب المشاكل
...
void onStart(ServiceInstance service) async {
  ...
  // تعريف الدالة مسبقاً لاستخدامها داخل attemptLocationUpdate
  late void Function() startRetryTimer;

  // 1. دالة محاولة التحديث
  Future<bool> _attemptLocationUpdate(
    ServiceInstance service,
    bool currentRetryMode,
  ...
  Future<void> startMainTimer() async {
    ...
    // تأكد أن الساعات لا تساوي صفر لتجنب المشاكل
    if (hours == 0) hours = 1;
...
Future<bool> _performLocationUpdate(ServiceInstance service) async {
  try {
    ...
    String locationAr = await _getFormattedAddress(
      position.latitude,
      position.longitude,
      "ar",
    );
    String locationEn = await _getFormattedAddress(
      position.latitude,
      position.longitude,
      "en",
    );
...
Future<bool> _sendLocationToServer({
  ...
  try {
    ...
    // تم التعديل: تحسين التحقق من استجابة السيرفر
    bool isHttpSuccess =
        response.statusCode != null &&
        response.statusCode! >= 200 &&
        response.statusCode! < 300;

    bool isBusinessSuccess = true;
    if (response.data is Map<String, dynamic> &&
        response.data.containsKey('status')) {
      // إذا كان الباك اند يرسل status داخل الـ body
      isBusinessSuccess =
...
Future<String> _getFormattedAddress(
  ...
  try {
    ...
    final response = await dio.get(
      ...
      queryParameters: {'format': 'json', 'lat': latitude, 'lon': longitude},
      options: Options(
        headers: {
