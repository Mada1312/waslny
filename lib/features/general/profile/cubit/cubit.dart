...
class ProfileCubit extends Cubit<ProfileState> {
  ...
  Future<void> logout(BuildContext context) async {
    try {
      ...
      // ✅ تحويل النوع إلى String للمقارنة الآمنة لضمان دخول الشرط
      String type = userModel.data?.userType?.toString() ?? "";

      if (type == '1') {
        // 1. إيقاف الـ Polling فوراً
        context.read<DriverHomeCubit>().stopPolling();

        // 2. محاولة جعل الكابتن Offline في السيرفر (اختياري)
        try {
          await api.updateStatus('0');
          log('Driver status set to offline before logout');
        } catch (e) {
          log('Failed to set driver offline: $e');
        }
      }

      // 3. تنفيذ طلب تسجيل الخروج من السيرفر
      final res = await api.logout();

      res.fold(
        (l) {
          // حتى لو فشل طلب السيرفر، يجب مسح البيانات محلياً للخروج
          errorGetBar(l.toString());
          _clearDataAndExit(context);
        },
        (r) {
          successGetBar(r.msg);
          _clearDataAndExit(context);
  ...
  // دالة مساعدة لتنظيف البيانات والخروج
  void _clearDataAndExit(BuildContext context) {
    Preferences.instance.clearUser();
    Navigator.pushNamedAndRemoveUntil(
      context,
      Routes.chooseLoginRoute,
