...
class ProfileRepo {
  ...
  // ✅ دالة تحديث حالة الكابتن
  Future<Either<Failure, DefaultPostModel>> updateStatus(String status) async {
    try {
      var response = await dio.post(
        EndPoints.toggleStatusUrl,
        body: {
          "status": status, // إرسال '0' لإيقاف الاتصال
        },
      );
