...
class UserShipmentsRepo {
  ...
  Future<Either<Failure, GetUserShipmentDetailsModel>> getShipmentDetails(
    ...
    try {
      final response = await api.get(
        EndPoints.shipmentDetailsUrl + id,
      );
  ...
  Future<Either<Failure, DefaultPostModel>> completeShipment(
    ...
    try {
      final response = await api.get(
        EndPoints.userCompleteShipmentUrl + id,
      );
  ...
  Future<Either<Failure, DefaultPostModel>> deleteShipment(
    ...
    try {
      final response = await api.get(
        EndPoints.deleteShipmentUrl + id,
      );
      return Right(DefaultPostModel.fromJson(response));
    } on ServerException {
      return Left(ServerFailure());
  ...
  Future<Either<Failure, DefaultPostModel>> assignDriver(
      {required String shipmentId, required String driverId}) async {
    try {
      final response = await api.post(EndPoints.assignDriverUrl, body: {
  ...
  Future<Either<Failure, DefaultPostModel>> updateShipmentStatus(
      ...
      double? long}) async {
    try {
      final response = await api.post(EndPoints.updateShipmentStatusUrl, body: {
        "shipment_id": shipmentId,
        "status": status,
        if (lat != null) "lat": lat,
